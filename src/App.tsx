...
import { buildAnimatedGifBlob, type AnimatedGifSourceFrame } from "./lib/animatedGif";
...
import type {
  ExportDialogState,
  ExportScope,
  ExportUpscaleAlgorithm,
  ExportUpscaleScale,
  ExportUpscaleSettings,
  ImportDialogState,
  ImportKind,
  LibraryImportMode,
  PhotoEdits,
...
const exportUpscaleScales = [1, 2, 3, 4, 6, 8] as const satisfies ReadonlyArray<ExportUpscaleScale>;
const exportUpscaleAlgorithmOptions = [
  { id: "nearest", label: "Nearest neighbor (preserves the pixelated low-resolution look)" },
  { id: "bilinear", label: "Bilinear (smoother, but blurrier edges)" },
  { id: "lanczos3", label: "Lanczos 3 (sharper, more detailed, may add slight halos)" },
] as const satisfies ReadonlyArray<{ id: ExportUpscaleAlgorithm; label: string }>;
...
interface GifExportDialogState {
  isOpen: boolean;
  frameDelayMs: number;
  pingPongLoop: boolean;
  upscaleScale: ExportUpscaleScale;
  upscaleAlgorithm: ExportUpscaleAlgorithm;
}
...
type OutputSizeHint = PixelSize | "mixed" | null;

interface UpscalePreviewState {
  url: string;
...
function getExportUpscaleSettings(state: Pick<ExportDialogState, "upscaleScale" | "upscaleAlgorithm">): ExportUpscaleSettings {
  return {
    scale: state.upscaleScale,
    algorithm: state.upscaleAlgorithm,
  };
}

function getGifExportUpscaleSettings(state: Pick<GifExportDialogState, "upscaleScale" | "upscaleAlgorithm">): ExportUpscaleSettings {
  return {
    scale: state.upscaleScale,
    algorithm: state.upscaleAlgorithm,
  };
...
function buildGifPlaybackSequence<T>(items: T[], pingPongLoop: boolean): T[] {
  if (!pingPongLoop || items.length <= 2) {
    return [...items];
  }
  return [...items, ...items.slice(1, -1).reverse()];
}

function getPhotoOutputSizeHint(photo: PhotoRecord): PixelSize | null {
  const frame = photo.preview ?? photo.thumbnail;
  return frame ? getTransformedFrameSize(frame, photo.edits) : null;
}

function getCommonOutputSizeHint(photos: PhotoRecord[]): OutputSizeHint {
  let commonSize: PixelSize | null = null;
  for (const photo of photos) {
    const size = getPhotoOutputSizeHint(photo);
    if (!size) {
      return null;
    }
    if (!commonSize) {
    ...
    if (commonSize.width !== size.width || commonSize.height !== size.height) {
      return "mixed";
    }
  }
  return commonSize;
...
function formatPixelSize(size: PixelSize) {
  return `${size.width} x ${size.height} px`;
}

function formatUpscaleScaleLabel(scale: ExportUpscaleScale, baseOutputSize: OutputSizeHint) {
  if (baseOutputSize === "mixed") {
    return scale === 1 ? "Original size (varies by photo)" : `${scale}x (varies by photo)`;
  }
  if (baseOutputSize) {
    return scale === 1
      ? `Original size (${formatPixelSize(baseOutputSize)})`
      : `${scale}x (${formatPixelSize({
        width: baseOutputSize.width * scale,
        height: baseOutputSize.height * scale,
      })})`;
  }
  return scale === 1 ? "Original size" : `${scale}x`;
}

function buildUpscalePreview(photo: PhotoRecord, upscale: ExportUpscaleSettings): UpscalePreviewState | null {
  const frame = photo.preview ?? photo.thumbnail;
  if (!frame || upscale.scale === 1) {
    return null;
  }

  const outputCanvas = createTransformedFrameCanvas(frame, photo.edits, upscale);
  const cropWidth = Math.min(420, outputCanvas.width);
  const cropHeight = Math.min(280, outputCanvas.height);
  const cropX = Math.floor((outputCanvas.width - cropWidth) / 2);
  const cropY = Math.floor((outputCanvas.height - cropHeight) / 2);
  ...
  context.drawImage(outputCanvas, cropX, cropY, cropWidth, cropHeight, 0, 0, cropWidth, cropHeight);
  return {
...
function buildAnimatedGifFileName(photos: PhotoRecord[]) {
  const firstPhoto = photos[0];
  if (!firstPhoto) {
    return "dj1000-animation.gif";
  }
  return `${buildIdentifiedExportStem(firstPhoto.relativePath || firstPhoto.name, firstPhoto.datBytes)}-animation.gif`;
}
        ...
        {openMenu === "file" && (
          <div className="window menu-dropdown">
            <div className="window-body context-menu-body">
              ...
              <div className="context-menu-separator" />
              <button className="context-menu-item" disabled={selectedCount < 2} onClick={runMenuAction(onGenerateGifSelected)}>
                Generate Animated GIF from Selected
              </button>
              {dngSupported ? (
                <>
                  <div className="context-menu-separator" />
...
function UpscalePreviewPanel({
  ...
  scale: ExportUpscaleScale;
}) {
  return (
    <div className="upscale-preview-panel">
      <div className="upscale-preview-copy">
        <strong>Upscale preview</strong>
        <span>
          Center crop from {photoName ?? "the first available image"}
          {preview ? `, output ${formatPixelSize(preview.outputSize)}` : ""}
        </span>
      </div>
      <div className="upscale-preview-image sunken-panel">
        {preview ? (
          <img src={preview.url} alt={`${scale}x upscale preview${photoName ? ` for ${photoName}` : ""}`} />
        ) : (
          <span>Preview will appear after a thumbnail or preview is ready.</span>
        )}
      </div>
    </div>
  );
...
function ExportDialog({
  ...
  baseOutputSize: OutputSizeHint;
  previewPhoto: PhotoRecord | null;
  desktopAvailable: boolean;
  dngSupported: boolean;
  ...
  const renderedFormat = getRenderedExportFormat(state.format);
  const canUpscaleRenderedOutput = renderedFormat !== null;
  const upscalePreview = canUpscaleRenderedOutput && previewPhoto
    ? buildUpscalePreview(previewPhoto, getExportUpscaleSettings(state))
    : null;
    ...
    <div className="dialog-backdrop">
      <div className="window dialog-window">
        ...
        <div className="window-body field-column">
          ...
          <div className="field-row-stacked">
            <label htmlFor="export-upscale-scale">Upscale rendered output</label>
            <select
              id="export-upscale-scale"
              value={state.upscaleScale}
              disabled={!canUpscaleRenderedOutput}
              onChange={(event) => onChange({
                ...state,
                upscaleScale: Number(event.target.value) as ExportUpscaleScale,
              })}
            >
              {exportUpscaleScales.map((scale) => (
                <option key={scale} value={scale}>
                  {formatUpscaleScaleLabel(scale, baseOutputSize)}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="field-row-stacked">
            <label htmlFor="export-upscale-algorithm">Upscaling algorithm</label>
            <select
              id="export-upscale-algorithm"
              value={state.upscaleAlgorithm}
              disabled={!canUpscaleRenderedOutput || state.upscaleScale === 1}
              onChange={(event) => onChange({
                ...state,
                upscaleAlgorithm: event.target.value as ExportUpscaleAlgorithm,
              })}
            >
              {exportUpscaleAlgorithmOptions.map((option) => (
                <option key={option.id} value={option.id}>{option.label}</option>
              ))}
            </select>
          </div>

          {state.upscaleScale > 1 && canUpscaleRenderedOutput ? (
            <UpscalePreviewPanel
              preview={upscalePreview}
              photoName={previewPhoto?.name ?? null}
              scale={state.upscaleScale}
            />
          ) : null}
          ...
          {formatNeedsDngSupport(state.format) ? (
            <p className="field-help">
              ...
              {state.format === "dng-png" || state.format === "dng-jpeg"
                ? " The companion .png or .jpg uses your current editor adjustments and any selected upscaling."
                : ""}
            </p>
          ) : null}
          {!canUpscaleRenderedOutput ? (
            <p className="field-help">Upscaling applies to PNG and JPEG exports only. RAW DNG output stays unedited.</p>
          ) : null}
...
function GifExportDialog({
  ...
  state: GifExportDialogState;
  photos: PhotoRecord[];
  baseOutputSize: OutputSizeHint;
  onClose: () => void;
  onChange: (next: GifExportDialogState) => void;
  onRun: () => void;
}) {
  const [previewIndex, setPreviewIndex] = useState(0);
  const previewPhotos = useMemo(
    () => buildGifPlaybackSequence(photos, state.pingPongLoop),
    [photos, state.pingPongLoop],
  );

  useEffect(() => {
    if (!state.isOpen || previewPhotos.length <= 1) {
      return;
    }
    const timer = window.setInterval(() => {
      setPreviewIndex((current) => (current + 1) % previewPhotos.length);
    }, Math.max(50, state.frameDelayMs));
    return () => window.clearInterval(timer);
  }, [previewPhotos.length, state.frameDelayMs, state.isOpen]);

  if (!state.isOpen) {
    return null;
  }

  const normalizedPreviewIndex = previewPhotos.length === 0 ? 0 : previewIndex % previewPhotos.length;
  const previewPhoto = previewPhotos[normalizedPreviewIndex] ?? null;
  const framesPerSecond = state.frameDelayMs > 0 ? 1000 / state.frameDelayMs : 0;
  const canGenerate = photos.length >= 2;

  return (
    <div className="dialog-backdrop">
      <div className="window dialog-window">
        <div className="title-bar">
          <div className="title-bar-text">Generate Animated GIF</div>
          <div className="title-bar-controls">
            <button aria-label="Close" onClick={onClose} />
          </div>
        </div>
        <div className="window-body field-column">
          <p className="field-help">
            The GIF will use the selected photos in the order you selected them.
            It uses the current Develop edits for each frame.
          </p>

          <div className="gif-preview-panel sunken-panel">
            {previewPhoto?.thumbnailUrl ? (
              <img src={previewPhoto.thumbnailUrl} alt={previewPhoto.name} />
            ) : (
              <span>{previewPhoto ? previewPhoto.name : "Select at least two photos"}</span>
            )}
          </div>
          <p className="field-help">
            Previewing frame {Math.min(normalizedPreviewIndex + 1, previewPhotos.length)} of {previewPhotos.length || 0}
            {previewPhoto ? `: ${previewPhoto.name}` : ""}
          </p>

          <div className="field-row">
            <input
              id="gif-ping-pong-loop"
              type="checkbox"
              checked={state.pingPongLoop}
              onChange={(event) => onChange({ ...state, pingPongLoop: event.target.checked })}
            />
            <label htmlFor="gif-ping-pong-loop">
              Ping-pong loop. Plays forward, then reverses through the middle frames before repeating.
            </label>
          </div>

          <div className="field-row-stacked">
            <label htmlFor="gif-frame-delay">
              Speed: {framesPerSecond.toFixed(framesPerSecond >= 10 ? 0 : 1)} fps ({state.frameDelayMs} ms per frame)
            </label>
            <input
              id="gif-frame-delay"
              type="range"
              ...
              max={2000}
              step={20}
              value={state.frameDelayMs}
              onChange={(event) => onChange({ ...state, frameDelayMs: Number(event.target.value) })}
            />
          </div>

          <div className="field-row-stacked">
            <label htmlFor="gif-upscale-scale">Upscale GIF</label>
            <select
              id="gif-upscale-scale"
              value={state.upscaleScale}
              onChange={(event) => onChange({
                ...state,
                upscaleScale: Number(event.target.value) as ExportUpscaleScale,
          ...
          <div className="field-row-stacked">
            <label htmlFor="gif-upscale-algorithm">Upscaling algorithm</label>
            <select
              id="gif-upscale-algorithm"
              value={state.upscaleAlgorithm}
              disabled={state.upscaleScale === 1}
              onChange={(event) => onChange({
                ...state,
                upscaleAlgorithm: event.target.value as ExportUpscaleAlgorithm,
              })}
          ...
          {!canGenerate ? (
            <p className="field-help">Select at least two photos to generate an animated GIF.</p>
          ) : null}

          <div className="dialog-actions">
            <button onClick={onRun} disabled={!canGenerate}>Generate GIF</button>
            <button onClick={onClose}>Cancel</button>
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  const [gifExportDialog, setGifExportDialog] = useState<GifExportDialogState>({
    isOpen: false,
    frameDelayMs: 500,
    pingPongLoop: false,
    upscaleScale: 1,
  ...
  const visiblePhotoById = useMemo(
    () => new Map(visiblePhotos.map((photo) => [photo.id, photo])),
    [visiblePhotos],
  );
  const selectedPhotos = useMemo(
    ...
    [selectedIds, visiblePhotoById],
  );
  const gifSelectedPhotos = useMemo(
    () => selectedPhotos.filter((photo) => !photo.metadata.removed),
    [selectedPhotos],
  );
  const exportDialogTargets = useMemo(() => {
    if (exportDialog.scope === "current") {
      return activePhoto ? [activePhoto] : [];
    }
    if (exportDialog.scope === "selected") {
      return selectedPhotos.filter((photo) => !photo.metadata.removed);
    }
    return sortedPhotos.filter((photo) => !photo.metadata.removed);
  }, [activePhoto, exportDialog.scope, selectedPhotos, sortedPhotos]);
  const exportBaseOutputSize = useMemo(
    () => getCommonOutputSizeHint(exportDialogTargets),
    [exportDialogTargets],
  );
  const gifBaseOutputSize = useMemo(
    () => getCommonOutputSizeHint(gifSelectedPhotos),
    [gifSelectedPhotos],
  );
  ...
  useEffect(() => {
    if (selectionAnchorRef.current && !visiblePhotoIds.has(selectionAnchorRef.current)) {
  ...
  function confirmClearCurrentLibrary() {
    ...
    setGifExportDialog((current) => ({ ...current, isOpen: false }));
  ...
  function handleSelectPhoto(photoId: string, event?: MouseEvent<HTMLButtonElement>) {
    ...
    if (event?.shiftKey) {
      const anchorId = selectionAnchorRef.current ?? previousActivePhotoId ?? photoId;
      const anchorIndex = visiblePhotos.findIndex((entry) => entry.id === anchorId);
      const targetIndex = visiblePhotos.findIndex((entry) => entry.id === photoId);
      if (anchorIndex !== -1 && targetIndex !== -1) {
        const startIndex = Math.min(anchorIndex, targetIndex);
        const endIndex = Math.max(anchorIndex, targetIndex);
        const rangeIds = visiblePhotos.slice(startIndex, endIndex + 1).map((entry) => entry.id);
        const shouldAddRange = event.metaKey || event.ctrlKey;
        setSelectedIds((current) => {
          const next = shouldAddRange ? new Set(current) : new Set<string>();
  ...
  function openGifExportDialog() {
    const targets = gifSelectedPhotos;
    if (targets.length < 2) {
      setStatus("Select at least two photos to generate an animated GIF.");
      return;
    }

    setOpenMenu(null);
    setGifExportDialog((current) => ({ ...current, isOpen: true }));
  }
  ...
  async function runExport() {
    ...
    try {
      ...
      const upscaleSettings = getExportUpscaleSettings(exportDialog);
      for (const [index, photo] of targets.entries()) {
        ...
        try {
          ...
          if (renderedFormat) {
            ...
            if (desktopBridge) {
              desktopFiles.push(...await buildDesktopExportPayload(photo, frame, renderedFormat, false, upscaleSettings));
            } else {
              browserFiles.push(...await buildBrowserExportBundle(photo, frame, renderedFormat, false, upscaleSettings));
            }
          }
  ...
  async function runGifExport() {
    ...
    const gifSettings = { ...gifExportDialog };
    const playbackTargets = buildGifPlaybackSequence(targets, gifSettings.pingPongLoop);
    const totalWork = targets.length + playbackTargets.length;
    setGifExportDialog((current) => ({ ...current, isOpen: false }));
    setStatus(`Preparing ${formatCount("photo", targets.length)} for animated GIF...`);
    setExportProgress({
      active: true,
      message: `Rendering 1 of ${targets.length}...`,
      completed: 0,
      total: totalWork,
      percent: 0,
    });
    ...
    try {
      const frames: AnimatedGifSourceFrame[] = [];
      for (const [index, photo] of targets.entries()) {
        setExportProgress({
          active: true,
          message: `Rendering ${photo.name} (${index + 1} of ${targets.length})...`,
          completed: index,
          total: totalWork,
          percent: Math.round((index / Math.max(totalWork, 1)) * 100),
        });
        const frame = await renderPoolRef.current!.render(photo.id, photo.edits, "export");
        frames.push({ frame, edits: photo.edits });
      }

      const playbackFrames = buildGifPlaybackSequence(frames, gifSettings.pingPongLoop);
      const gifBlob = await buildAnimatedGifBlob(playbackFrames, {
        delayMs: gifSettings.frameDelayMs,
        upscale: getGifExportUpscaleSettings(gifSettings),
        onProgress: (encodedFrames, totalFrames) => {
          const completed = targets.length + encodedFrames;
          setExportProgress({
            active: true,
            message: `Encoding GIF frame ${encodedFrames} of ${totalFrames}...`,
            completed,
            total: totalWork,
            percent: Math.round((completed / Math.max(totalWork, 1)) * 100),
          });
        },
      });

      const fileName = buildAnimatedGifFileName(targets);
      setExportProgress({
        active: true,
        message: "Writing animated GIF...",
        completed: totalWork,
        total: totalWork,
        percent: 100,
      });
      ...
      if (desktopBridge) {
        await desktopBridge.exportFiles({
          files: [
            {
              relativePath: fileName,
              bytes: await gifBlob.arrayBuffer(),
            },
          ],
          suggestedFolderName: "dj1000-gif-export",
        });
      } else {
        triggerBrowserDownload(gifBlob, fileName);
      }

      setStatus(`Generated animated GIF from ${formatCount("photo", targets.length)}.`);
    } catch (error) {
      setStatus(`Animated GIF export failed: ${String(error instanceof Error ? error.message : error)}`);
      openExportIssuesDialog(
        "Animated GIF Export Failed",
        "The selected photos could not be exported as an animated GIF.",
  ...
  return (
    <div className="app-shell">
      ...
      <ExportDialog
        state={exportDialog}
        baseOutputSize={exportBaseOutputSize}
        previewPhoto={exportDialogTargets[0] ?? null}
        desktopAvailable={!!desktopBridge}
      ...
      <GifExportDialog
        state={gifExportDialog}
        photos={gifSelectedPhotos}
        baseOutputSize={gifBaseOutputSize}
        onClose={() => setGifExportDialog((current) => ({ ...current, isOpen: false }))}
        onChange={setGifExportDialog}
        onRun={() => void runGifExport()}
      />
      ...
      <div className="desktop-frame window" style={{height: "100%"}}>
        ...
        <div ref={shellToolbarRef} className="shell-toolbar">
          <AppMenu
            ...
            canPasteEdits={copiedEdits !== null}
            dngSupported={dngSupported}
            selectedCount={gifSelectedPhotos.length}
            openMenu={openMenu}
            ...
            onGenerateGifSelected={openGifExportDialog}
            ...
            onSelectAll={() => {
              ...
              selectionAnchorRef.current = photoIds[0] ?? null;
              ...
              {openMenu === "export" && (
                <div className={`window menu-dropdown${toolbarMenuAlignment.export ? " menu-dropdown-align-right" : ""}`}>
                  <div className="window-body context-menu-body">
                    ...
                    <button
                      className="context-menu-item"
                      disabled={gifSelectedPhotos.length < 2}
