...
export async function renderFrameToBlob(
  ...
  const canvas = createTransformedFrameCanvas(frame, edits, upscale);
...
export async function renderFrameToDownloadFile(
  ...
  const blob = await renderFrameToBlob(frame, edits, format, upscale);
  return {
...
export async function buildBrowserExportBundle(
  ...
  const rendered = await renderFrameToDownloadFile(frame, photo.edits, format, stem, upscale);
